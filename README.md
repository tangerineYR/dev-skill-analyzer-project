...
데이터사이언스학을 전공하며 데이터를 통해 문제를 해결하는 방법을 배우고 있지만, 정작 우리 자신의 진로 문제 앞에서는 데이터가 아닌 막연한 소문에 의존하고 있음을 발견했습니다. 백엔드, 프론트엔드 등 각 개발 직무가 실제 기업에서 어떤 기술 스택을 요구하는지에 대한 **객관적인 정보가 부족** 하기 때문입니다.
...
- 데이터 수집 (Scraping): scraper.py
  ...
  공고 '본문 텍스트'와 '기술 스택 태그'를 모두 수집하여 누락되는 데이터를 최소화합니다.
...
분석 결과 요약: 수집된 유효 공고 수(N)는 서버 개발자(N=44), 프론트엔드 개발자(N=46), 데이터 사이언티스트(N=29)로, 신입 데이터 사이언티스트 시장이 상대적으로 작음을 확인했습니다.

#### 분석 1: 직무별 핵심 요구 기술 (가로 막대그래프)

각 직무의 '필수 기술'을 파악하기 위해 상위 기술 스택의 요구 비율을 시각화했습니다. 

- 서버 개발자: **‘Java/AWS’** 가 핵심. Python, SQL, Spring이 뒤를 이음.

- 프론트엔드 개발자: **‘React/TypeScript’** 가 압도적. Next.js, JavaScript가 주요 기술.

- 데이터 사이언티스트: **‘Python/PyTorch’** 가 핵심. TensorFlow, SQL이 뒤를 이음.
...
세 직무가 공유하는 기술과 각 직무에 특화된 기술을 비교하기 위해 히트맵을 생성했습니다.

- 공유 기반 기술:

  Git은 세 직무 모두에서 11~24%의 비율을 보이며, 직군 공통의 기본 소양임을 확인했습니다. SQL 역시 DS(20.7%)와 서버(22.7%) 직군에서 중요하게 요구되었습니다.

- 직무 특화 기술:

  React, TypeScript는 프론트엔드 개발자 행만 압도적으로 진한 색을 보였습니다.

  PyTorch, TensorFlow는 데이터 사이언티스트 행만 진한 색을 보였습니다.

  Java, Spring은 서버 개발자 행이 가장 진한 색을 보였습니다.
